...
import { ArrowLeft, ArrowRight, Check, Headphones, Landmark, Loader2, ShieldCheck, Sparkles, UserRound, Volume2, X } from 'lucide-react'
...
export default function CustomerPortal({ onBack }) {
  ...
  const processText = async (text) => {
    ...
    try {
      const res = await customerFetch('/customer/chat', {
        ...
        body: JSON.stringify({ session_id: sessionId, text, language, complexity: 'simple', elderly_mode: elderlyMode })
      })
      if (!res.ok) throw new Error('chat')
      const data = await res.json()
      setMessages((m) => [...m, { role: 'assistant', text: data.reply_local, visualData: data.visual_data, uiAction: data.ui_action }])
  ...
  const reset = () => { clearCustomerSession(); setLanguage(null); setSessionId(null); setMessages([]); setEnded(false); setStatusKey(''); setShowEligibility(false) }
...
function CustomerBubble({ m, tr, onOpenEligibility }) {
  ...
  return <div className={`flex ${mine?'justify-end':'justify-start'}`}><div className={`max-w-[88%] rounded-2xl px-4 py-3 ${mine?'bg-primary-500 text-white':'bg-primary-50 text-charcoal border border-primary-100'}`}><p className="text-xs font-bold opacity-70 mb-1">{mine?tr('you'):tr('assistant')}</p><p>{m.text}</p>{m.uiAction==='open_eligibility' && <button onClick={onOpenEligibility} className="mt-3 w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-primary-500 text-white font-semibold"><Landmark className="h-4 w-4" />{tr('eligOffer')}</button>}</div></div>
}
