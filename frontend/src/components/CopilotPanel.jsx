...
export default function CopilotPanel() {
  ...
  const speakReply = async () => {
    ...
    // /api/speak filters sensitive content unconditionally regardless of
    // this text's source — including if the employee edited it — so this
    // is safe even though editedReply may differ from result.spoken_response.
  ...
  return (
    <div className="grid lg:grid-cols-5 gap-6">
      ...
      <div className="lg:col-span-2">
        <div className="premium-card p-7 lg:sticky lg:top-28">
          ...
          <form onSubmit={askCopilot} className="space-y-5">
            <div>
              ...
              <textarea
                ...
                className="w-full border border-primary-100 rounded-2xl shadow-[0_1px_0_rgba(255,255,255,.8)_inset] px-4 py-3 text-base
                  focus:border-primary-400 focus:ring-4 focus:ring-primary-100/60 outline-none transition-all resize-none"
              />
            </div>
            ...
            <button
              type="submit"
              disabled={busy}
              className="w-full py-3.5 rounded-2xl bg-primary-500 text-white font-display font-semibold shadow-soft hover:shadow-card text-lg
      ...
      <div className="lg:col-span-3 space-y-5">
        {!result && !busy && (
          <div className="premium-card p-10 text-center text-charcoal/50">
        ...
        {result && (
          <>
            ...
            {result.visual_data && (
              <VisualDataCard data={result.visual_data} />
            )}
            ...
            <div className="bg-white rounded-3xl shadow-card border border-secondary-400/50 p-7">
              ...
              {result.sensitive && (
                <div className="flex items-center gap-2 text-warn-600 bg-warn-100 rounded-xl px-4 py-2.5 text-sm mb-4">
                  <ShieldAlert className="h-4 w-4 shrink-0" />
                  This response involves sensitive financial info — it will never be spoken aloud, only shown on screen.
                </div>
              )}
              <textarea
                value={editedReply}
                ...
                className="w-full border-2 border-secondary-400/60 rounded-xl px-4 py-3 text-base
                  focus:border-primary-400 focus:ring-4 focus:ring-primary-100/60 outline-none transition-all resize-none bg-secondary-50"
              />
              ...
              <button
                onClick={speakReply}
                className="mt-4 px-6 py-3 rounded-2xl bg-secondary-500 text-charcoal font-display font-semibold shadow-soft
