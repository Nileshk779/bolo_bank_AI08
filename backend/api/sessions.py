...
@router.get("/{session_id}/summary", response_model=SessionSummaryResponse, dependencies=[Depends(limit_staff_ai)])
def session_summary(session_id: str, staff: str = Depends(get_current_staff), db: Session = Depends(get_db)):
